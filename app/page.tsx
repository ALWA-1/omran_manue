...
import { 
  ...
  Loader2, MessageCircle, ShoppingBag, Plus, X, Share, MoreHorizontal,
...
export default function Home() {
  ...
  // اكتشاف أجهزة iOS وحالة التثبيت
  useEffect(() => {
  ...
  // منطق التمرير التلقائي للناف بار العلوي
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <main className="min-h-screen pb-10 font-sans selection:bg-red-200 relative" style={{ backgroundColor: '#F2ECE4', direction: isAr ? 'rtl' : 'ltr' }}>
            ...
            <div key={cat.id} ref={(el) => { sectionRefs.current[cat.id] = el; }} className="mb-10 pt-4">
              ...
              <div className={`grid gap-6 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
                {itemsInCategory.map(item => (
                  <div key={item.id} onClick={() => setSelectedItem(item)} className={`cursor-pointer group relative ${viewMode === 'list' ? 'flex items-start gap-4 mb-2' : 'flex flex-col'}`}>
                    ...
                    <div className={`flex flex-col flex-grow ${viewMode === 'grid' ? 'items-center text-center px-2' : 'py-1'}`}>
                      ...
                      <div className={`mt-auto w-full flex items-center ${viewMode === 'list' ? 'justify-between' : 'justify-center gap-4'}`}>
                        <div className={`bg-[#84796B] text-white text-[13px] font-bold px-3 py-1.5 rounded-lg shadow-sm`}>
                          {item.price} {currentCurrency}
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); addToCart(item); }} className="flex items-center justify-center gap-1.5 bg-[#C8102E] hover:bg-[#A00D24] text-white px-4 py-1.5 rounded-lg text-[13px] font-bold shadow-sm active:scale-95 transition-transform">
      ...
      {/* --- مودال إرشادات تثبيت iOS الجديد --- */}
      <AnimatePresence>
        {showIOSModal && (
          <motion.div 
            initial={{ opacity: 0 }} 
          ...
          >
            ...
            >
              <button onClick={() => setShowIOSModal(false)} className="absolute top-4 right-4 bg-[#F2ECE4] text-[#84796B] hover:text-[#C8102E] p-2 rounded-full transition-colors"><X size={20}/></button>
              
              <div className="text-center mb-6 mt-2">
                <div className="w-16 h-16 bg-[#F2ECE4] text-[#C8102E] rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-sm">
                   <Download size={32} />
                </div>
                <h2 className="text-2xl font-black text-[#222222]">{isAr ? 'تثبيت التطبيق' : 'Install App'}</h2>
                <p className="text-[#84796B] text-sm font-medium mt-2">{isAr ? 'تابع الخطوات التالية لتثبيت التطبيق على جهازك:' : 'Follow these steps to install the app:'}</p>
              </div>

              <div className="flex flex-col gap-4">
                {/* خطوة 1 */}
                <div className="flex items-center gap-4 bg-[#F2ECE4] p-3.5 rounded-2xl">
                  <div className="w-10 h-10 shrink-0 bg-white border border-[#E8E2D9] rounded-full flex items-center justify-center shadow-sm">
                    <MoreHorizontal size={22} className="text-[#222222]" />
                  </div>
                  <p className="text-[#222222] font-bold text-sm leading-snug">{isAr ? '1. اضغط على أيقونة الثلاث نقاط في أسفل الموقع' : '1. Tap the three dots icon at the bottom'}</p>
                </div>

                {/* خطوة 2 */}
                <div className="flex items-center gap-4 bg-[#F2ECE4] p-3.5 rounded-2xl">
                  <div className="w-10 h-10 shrink-0 bg-white border border-[#E8E2D9] rounded-full flex items-center justify-center shadow-sm">
                    <Share size={18} className="text-[#222222]" />
                  </div>
                  <p className="text-[#222222] font-bold text-sm leading-snug">{isAr ? '2. اختر زر "مشاركة" من القائمة الظاهرة' : '2. Select the "Share" button from the menu'}</p>
                </div>
                ...
                <div className="flex items-center gap-4 bg-[#F2ECE4] p-3.5 rounded-2xl">
                  ...
                  <p className="text-[#222222] font-bold text-sm leading-snug">{isAr ? '3. انزل لأسفل واختر "إضافة إلى الصفحة الرئيسية"' : '3. Scroll down and select "Add to Home Screen"'}</p>
                </div>

                {/* خطوة 4 */}
                <div className="flex items-center gap-4 bg-[#F2ECE4] p-3.5 rounded-2xl">
                  <div className="w-10 h-10 shrink-0 bg-[#C8102E] text-white rounded-full flex items-center justify-center shadow-md font-bold text-xs">إضافة</div>
                  <p className="text-[#222222] font-bold text-sm leading-snug">{isAr ? '4. اضغط على زر "إضافة" الموجود أعلى الشاشة' : '4. Tap the "Add" button at the top right'}</p>
                </div>
              </div>
              
              <button onClick={() => setShowIOSModal(false)} className="w-full mt-6 py-3.5 bg-[#222222] hover:bg-black text-white rounded-xl font-bold text-[15px] shadow-md transition-colors">
      ...
      {/* زر سلة المشتريات العائم */}
      <AnimatePresence>
        {totalItems > 0 && !isCartOpen && (
          <motion.button initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} onClick={() => setIsCartOpen(true)} className="fixed bottom-40 right-6 md:right-10 z-50 bg-[#C8102E] text-white p-4 md:p-5 rounded-full shadow-[0_8px_25px_rgba(200,16,46,0.4)] hover:scale-110 active:scale-95 transition-all flex items-center justify-center">
